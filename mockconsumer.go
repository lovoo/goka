...
// MockConsumer implements sarama's Consumer interface for testing purposes.
...
type MockConsumer struct {
	...
	partitionConsumers map[string]map[int32]*MockPartitionConsumer
	metadata           map[string][]int32
}

// NewMockConsumer returns a new mock Consumer instance. The t argument should
// be the *testing.T instance of your test method. An error will be written to it if
// an expectation is violated. The config argument can be set to nil.
func NewMockConsumer(t *testing.T, config *sarama.Config) *MockConsumer {
	if config == nil {
	...
	c := &MockConsumer{
		t:                  t,
		config:             config,
		partitionConsumers: make(map[string]map[int32]*MockPartitionConsumer),
	}
	return c
}
...
func (c *MockConsumer) SetTopicMetadata(metadata map[string][]int32) {
	c.l.Lock()
	defer c.l.Unlock()
...
func (c *MockConsumer) ExpectConsumePartition(topic string, partition int32, offset int64) *MockPartitionConsumer {
	c.l.Lock()
	defer c.l.Unlock()

	if c.partitionConsumers[topic] == nil {
		c.partitionConsumers[topic] = make(map[int32]*MockPartitionConsumer)
	}

	if c.partitionConsumers[topic][partition] == nil {
		c.partitionConsumers[topic][partition] = &MockPartitionConsumer{
			t:         c.t,
			topic:     topic,
			partition: partition,
			offset:    offset,
...
// expectation is not met, an error is reported to the error reporter.
func (pc *MockPartitionConsumer) ExpectErrorsDrainedOnClose() {
