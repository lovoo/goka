...
// Context2 provides access to the processor's table and emit capabilities to
...
func (ctx *cbContext2) Emit(topic Stream, key string, value interface{}) {
	if topic == "" {
...
func (ctx *cbContext2) Loopback(key string, value interface{}) {
...
func (ctx *cbContext2) emit(topic string, key string, value []byte) {
	ctx.counters.emits++
...
// Value returns the value of the key in the group table.
func (ctx *cbContext2) Value() interface{} {
...
// SetValue updates the value of the key in the group table.
func (ctx *cbContext2) SetValue(value interface{}) {
...
func (ctx *cbContext2) Timestamp() time.Time {
	return ctx.msg.Timestamp
}

func (ctx *cbContext2) Key() string {
	return string(ctx.msg.Key)
}

func (ctx *cbContext2) Topic() Stream {
	return Stream(ctx.msg.Topic)
}

func (ctx *cbContext2) Offset() int64 {
	return ctx.msg.Offset
}

func (ctx *cbContext2) Partition() int32 {
	return ctx.msg.Partition
}

func (ctx *cbContext2) Join(topic Table) interface{} {
	if ctx.pviews == nil {
...
func (ctx *cbContext2) Lookup(topic Table, key string) interface{} {
	if ctx.views == nil {
...
func (ctx *cbContext2) valueForKey(key string) (interface{}, error) {
	if ctx.table == nil {
...
func (ctx *cbContext2) deleteKey(key string) error {
...
func (ctx *cbContext2) setValueForKey(key string, value interface{}) error {
	if ctx.graph.GroupTable() == nil {
		return fmt.Errorf("Cannot access state in stateless processor")
	}
...
func (ctx *cbContext2) emitDone(err error) {
	ctx.m.Lock()
	defer ctx.m.Unlock()
	ctx.counters.dones++
	ctx.tryCommit(err)
...
func (ctx *cbContext2) finish(err error) {
	ctx.m.Lock()
	defer ctx.m.Unlock()
	ctx.done = true
	ctx.tryCommit(err)
...
func (ctx *cbContext2) tryCommit(err error) {
	if err != nil {
...
// Fail stops execution and shuts down the processor
func (ctx *cbContext2) Fail(err error) {
	panic(err)
}

func (ctx *cbContext2) Context() context.Context {
	return ctx.ctx
}
