package templates

import (
	"bytes"
	"compress/gzip"
	"fmt"
	"io"
	"strings"
)

func bindata_read(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	return buf.Bytes(), nil
}

var _web_templates_common_base_go_html = []byte("")

func web_templates_common_base_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_common_base_go_html,
		"web/templates/common/base.go.html",
	)
}

var _web_templates_common_head_go_html = []byte("")

func web_templates_common_head_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_common_head_go_html,
		"web/templates/common/head.go.html",
	)
}

var _web_templates_common_menu_go_html = []byte("")

func web_templates_common_menu_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_common_menu_go_html,
		"web/templates/common/menu.go.html",
	)
}

var _web_templates_index_index_go_html = []byte("")

func web_templates_index_index_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_index_index_go_html,
		"web/templates/index/index.go.html",
	)
}

var _web_templates_monitor_details_go_html = []byte("")

func web_templates_monitor_details_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_monitor_details_go_html,
		"web/templates/monitor/details.go.html",
	)
}

var _web_templates_monitor_index_go_html = []byte("")

func web_templates_monitor_index_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_monitor_index_go_html,
		"web/templates/monitor/index.go.html",
	)
}

var _web_templates_monitor_menu_go_html = []byte("")

func web_templates_monitor_menu_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_monitor_menu_go_html,
		"web/templates/monitor/menu.go.html",
	)
}

var _web_templates_query_index_go_html = []byte("")

func web_templates_query_index_go_html() ([]byte, error) {
	return bindata_read(
		_web_templates_query_index_go_html,
		"web/templates/query/index.go.html",
	)
}

// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		return f()
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// AssetNames returns the names of the assets.
func AssetNames() []string {
	names := make([]string, 0, len(_bindata))
	for name := range _bindata {
		names = append(names, name)
	}
	return names
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string]func() ([]byte, error){
	"web/templates/common/base.go.html": web_templates_common_base_go_html,
	"web/templates/common/head.go.html": web_templates_common_head_go_html,
	"web/templates/common/menu.go.html": web_templates_common_menu_go_html,
	"web/templates/index/index.go.html": web_templates_index_index_go_html,
	"web/templates/monitor/details.go.html": web_templates_monitor_details_go_html,
	"web/templates/monitor/index.go.html": web_templates_monitor_index_go_html,
	"web/templates/monitor/menu.go.html": web_templates_monitor_menu_go_html,
	"web/templates/query/index.go.html": web_templates_query_index_go_html,
}
// AssetDir returns the file names below a certain
// directory embedded in the file by go-bindata.
// For example if you run go-bindata on data/... and data contains the
// following hierarchy:
//     data/
//       foo.txt
//       img/
//         a.png
//         b.png
// then AssetDir("data") would return []string{"foo.txt", "img"}
// AssetDir("data/img") would return []string{"a.png", "b.png"}
// AssetDir("foo.txt") and AssetDir("notexist") would return an error
// AssetDir("") will return []string{"data"}.
func AssetDir(name string) ([]string, error) {
	node := _bintree
	if len(name) != 0 {
		cannonicalName := strings.Replace(name, "\\", "/", -1)
		pathList := strings.Split(cannonicalName, "/")
		for _, p := range pathList {
			node = node.Children[p]
			if node == nil {
				return nil, fmt.Errorf("Asset %s not found", name)
			}
		}
	}
	if node.Func != nil {
		return nil, fmt.Errorf("Asset %s not found", name)
	}
	rv := make([]string, 0, len(node.Children))
	for name := range node.Children {
		rv = append(rv, name)
	}
	return rv, nil
}

type _bintree_t struct {
	Func func() ([]byte, error)
	Children map[string]*_bintree_t
}
var _bintree = &_bintree_t{nil, map[string]*_bintree_t{
	"web": &_bintree_t{nil, map[string]*_bintree_t{
		"templates": &_bintree_t{nil, map[string]*_bintree_t{
			"common": &_bintree_t{nil, map[string]*_bintree_t{
				"base.go.html": &_bintree_t{web_templates_common_base_go_html, map[string]*_bintree_t{
				}},
				"head.go.html": &_bintree_t{web_templates_common_head_go_html, map[string]*_bintree_t{
				}},
				"menu.go.html": &_bintree_t{web_templates_common_menu_go_html, map[string]*_bintree_t{
				}},
			}},
			"index": &_bintree_t{nil, map[string]*_bintree_t{
				"index.go.html": &_bintree_t{web_templates_index_index_go_html, map[string]*_bintree_t{
				}},
			}},
			"monitor": &_bintree_t{nil, map[string]*_bintree_t{
				"details.go.html": &_bintree_t{web_templates_monitor_details_go_html, map[string]*_bintree_t{
				}},
				"index.go.html": &_bintree_t{web_templates_monitor_index_go_html, map[string]*_bintree_t{
				}},
				"menu.go.html": &_bintree_t{web_templates_monitor_menu_go_html, map[string]*_bintree_t{
				}},
			}},
			"query": &_bintree_t{nil, map[string]*_bintree_t{
				"index.go.html": &_bintree_t{web_templates_query_index_go_html, map[string]*_bintree_t{
				}},
			}},
		}},
	}},
}}
